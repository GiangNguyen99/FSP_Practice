...
baud_setting_t g_uart0_baud_setting =
        ...
        /* Baud rate calculated with 0.009% error. */.semr_baudrate_bits_b.abcse = 0,
          .semr_baudrate_bits_b.abcs = 0, .semr_baudrate_bits_b.bgdm = 1, .cks = 0, .brr = 43, .mddr = (uint8_t) 173, .semr_baudrate_bits_b.brme =
                  true };
...
{ .unit = 0, .mode = ADC_MODE_CONTINUOUS_SCAN, .resolution = ADC_RESOLUTION_12_BIT, .alignment =
  ...
  .trigger = ADC_TRIGGER_SOFTWARE, .p_callback = user_adc_callback,
...
{
    .compare_mask        = ADC_MASK_TEMPERATURE |  0,
    .compare_mode_mask   =  0,
    .compare_cfg         = (ADC_COMPARE_CFG_WINDOW_ENABLE) | (0) | (0) | (ADC_COMPARE_CFG_EVENT_OUTPUT_OR),
    .compare_ref_low     = 20,
    .compare_ref_high    = 30,
...
gpt_instance_ctrl_t g_timer3_pwm_ctrl;
#if 0
const gpt_extended_pwm_cfg_t g_timer3_pwm_pwm_extend =
{
...
#endif
const gpt_extended_cfg_t g_timer3_pwm_extend =
        { .gtioca =
...
#if 0
    .p_pwm_cfg                   = &g_timer3_pwm_pwm_extend,
#else
          .p_pwm_cfg = NULL,
#endif
#if 0
...
const timer_cfg_t g_timer3_pwm_cfg =
{ .mode = TIMER_MODE_PWM,
/* Actual period: 0.0008333333333333334 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x186a0,
  .duty_cycle_counts = 0xc350, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
/* Instance structure to use this module. */
const timer_instance_t g_timer3_pwm =
{ .p_ctrl = &g_timer3_pwm_ctrl, .p_cfg = &g_timer3_pwm_cfg, .p_api = &g_timer_on_gpt };
...
const gpt_extended_cfg_t g_timer2_extend =
        ...
        { .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW },
          ...
          .count_up_source = (gpt_source_t) (GPT_SOURCE_NONE), .count_down_source = (gpt_source_t) (GPT_SOURCE_NONE), .capture_a_source =
                  (gpt_source_t) (GPT_SOURCE_NONE),
          .capture_b_source = (gpt_source_t) (GPT_SOURCE_NONE), .capture_a_ipl = (10), .capture_b_ipl =
                  (BSP_IRQ_DISABLED),
...
/* Actual period: 5 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x23c34600,
  .duty_cycle_counts = 0x11e1a300, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback =
          user_timer2_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
/* Actual calculated bitrate: 394737. Actual calculated duty cycle: 50%. */.clock_settings.brl_value = 26,
  .clock_settings.brh_value = 26, .clock_settings.cks_value = 1, };
const i2c_master_cfg_t g_i2c_master0_cfg =
{ .channel = 0, .rate = I2C_MASTER_RATE_FAST, .slave = 0x68, .addr_mode = I2C_MASTER_ADDR_MODE_7BIT,
#define RA_NOT_DEFINED (1)
#if (RA_NOT_DEFINED == RA_NOT_DEFINED)
  .p_transfer_tx = NULL,
#else
